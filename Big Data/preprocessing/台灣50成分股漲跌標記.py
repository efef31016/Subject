...
def create_df(date,stock_no):
    ...
    s2 = s.drop(['date'],axis = 1)  #刪除日期欄位，也可以直接s.set_index("date", inplace=True)
    mlist = []
...
st = 2010
num = 10
listDji = ['1326','2327','2885','2379','2382','2892','1101','2357','2207','2880','3045','4938','2912','2395','2474','2887','2883','6505','2890','2801','1402','4904','1101','9910','2105','2408','2330','4904','1101','9910','2105','2408'] #,'3034','3711','5871','5880','6515','5876','6669','2633',聯詠抓不到
df = pd.DataFrame()
...
for i in range(len(listDji)):
  for j in range(st, st+num):
    for k in range(1,13):
      ...
      print(result)
      df = df.append(result)   # list 與 DataFrame 的 .append不一樣(DataFrame 要另設變數)
  
  df.to_csv('%s.csv' % int(listDji[i]), index=True)